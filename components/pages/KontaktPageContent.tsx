import Link from 'next/link'
import { contentApi, type ContentApi } from '@/lib/content'
import { telHref, mailHref, fillTemplate } from '@/lib/utils'
import { PageHeader } from '@/components/shared/PageHeader'
import { BreadcrumbSchema } from '@/lib/schema'
import { Phone, Mail, MapPin } from 'lucide-react'
import { AnimatedReveal } from '@/components/shared/AnimatedReveal'

export async function KontaktPageContent({ api = contentApi }: { api?: ContentApi } = {}) {
  const [cms, site] = await Promise.all([api.getKontaktPageContent(), api.getSiteSettings()])

  const mapsUrl =
    site.googleMapsUrl ||
    `https://maps.google.com/?q=${encodeURIComponent(`${site.address}, ${site.postcode} ${site.city}`)}`

  const cards = [
    {
      icon: <Phone size={22} strokeWidth={1.5} />,
      label: cms.phoneLabel,
      main: site.phone,
      href: telHref(site.phone),
      sub: cms.phoneNote || site.hours,
      cta: cms.phoneCta,
      external: false,
    },
    {
      icon: <Mail size={22} strokeWidth={1.5} />,
      label: cms.emailLabel,
      main: site.email,
      href: mailHref(site.email),
      sub: cms.emailNote,
      cta: cms.emailCta,
      external: false,
    },
    {
      icon: <MapPin size={22} strokeWidth={1.5} />,
      label: cms.addressLabel,
      main: site.address,
      href: mapsUrl,
      sub: `${site.postcode} ${site.city}`,
      cta: cms.addressCta,
      external: true,
    },
  ]

  return (
    <>
      <BreadcrumbSchema items={[{ name: 'Kontakt', href: '/kontakt' }]} />

      <div className="bg-stone-light border-b border-stone-border">
        <PageHeader
          label={cms.pageLabel}
          title={cms.pageTitle}
          lead={fillTemplate(cms.pageLead, {
            godziny: site.hours,
            telefon: site.phone,
            email: site.email,
            adres: site.address,
            miasto: site.city,
          })}
        />
      </div>

      <section className="bg-stone-bg py-section-md">
        <div className="container-stone">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-stone-border">
            {cards.map((item, i) => (
              <AnimatedReveal key={item.label} delay={i * 80}>
                <div className="bg-stone-bg p-8 lg:p-10 h-full flex flex-col gap-5">
                  <div
                    className="w-12 h-12 flex items-center justify-center"
                    style={{ background: 'rgba(196,184,122,0.12)', color: 'var(--color-gold-dark)' }}
                    aria-hidden="true"
                  >
                    {item.icon}
                  </div>
                  <div>
                    <p
                      style={{ fontFamily: 'var(--font-body)', fontSize: '9px', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--color-text-3)', marginBottom: '8px' }}
                    >
                      {item.label}
                    </p>
                    <a
                      href={item.href}
                      className="block font-display text-[22px] text-ink hover:text-[#A89B58] transition-colors duration-200 leading-[1.2] mb-2 break-words"
                      style={{ fontWeight: 400 }}
                      target={item.external ? '_blank' : undefined}
                      rel={item.external ? 'noopener noreferrer' : undefined}
                    >
                      {item.main}
                    </a>
                    {item.sub && <p className="text-[13px] text-ink-secondary">{item.sub}</p>}
                  </div>
                  <a
                    href={item.href}
                    className="link-stone mt-auto self-start"
                    target={item.external ? '_blank' : undefined}
                    rel={item.external ? 'noopener noreferrer' : undefined}
                  >
                    {item.cta} →
                  </a>
                </div>
              </AnimatedReveal>
            ))}
          </div>

          <AnimatedReveal delay={200} className="mt-16 text-center">
            <p className="font-display text-display-sm text-ink mb-4" style={{ fontWeight: 400 }}>
              {cms.bottomText}
            </p>
            <Link href={cms.bottomHref} className="btn-primary">
              {cms.bottomButton}
            </Link>
          </AnimatedReveal>
        </div>
      </section>
    </>
  )
}
