...
export function RealizationGallery() {
  ...
  return (
    <section className="bg-stone-bg py-section-md">
      <div className="container-stone">
        ...
        {hasPhotos ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-1 bg-stone-border">
            {photos.map((photo, i) => (
              <AnimatedReveal key={photo.file} delay={i * 70}>
                <button
          ...
          <div
            className="grid grid-cols-2 lg:grid-cols-3 gap-1 bg-stone-border"
            aria-hidden="true"
          >
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="aspect-stone bg-stone-light"
      ...
      {lightboxIndex !== null && (
        <Lightbox
          images={photos.map((p) => ({ src: photoSrc(p.file), alt: p.alt }))}
