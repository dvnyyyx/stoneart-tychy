    ...
    oNas: singleton({
      ...
      schema: {
        pageTitle: fields.text({
          ...
          defaultValue: 'StoneArt — Tychy.',
          validation: { isRequired: true },
        }),
        pageLead: fields.text({
          label: 'Nagłówek — lead',
          multiline: true,
          defaultValue: 'Specjalizujemy się w liternictwie nagrobnym, piaskowaniu napisów oraz renowacji nagrobków i tablic granitowych. Działamy na terenie Tychów i okolicznych miejscowości.',
          validation: { isRequired: true },
        }),
        s1Label: fields.text({ label: 'Sekcja 1 — etykieta', defaultValue: 'Czym się zajmujemy' }),
        s1Title: fields.text({ label: 'Sekcja 1 — tytuł', defaultValue: 'Liternictwo, dopiski i renowacja nagrobków.', validation: { isRequired: true } }),
        s1Text1: fields.text({ label: 'Sekcja 1 — akapit 1', multiline: true, defaultValue: 'Zajmujemy się wykonywaniem nowych napisów, dopisków liter i dat, odświeżaniem istniejących inskrypcji oraz pracami związanymi z odnawianiem nagrobków. Pracujemy na różnych rodzajach kamienia naturalnego, dbając o odpowiedni dobór techniki oraz zachowanie czytelności i stylu napisów.', validation: { isRequired: true } }),
        s1Text2: fields.text({ label: 'Sekcja 1 — akapit 2', multiline: true, defaultValue: 'Wykonujemy usługi zarówno dla klientów indywidualnych, jak i zakładów kamieniarskich.' }),
        s2Label: fields.text({ label: 'Sekcja 2 — etykieta', defaultValue: 'Podejście do pracy' }),
        s2Title: fields.text({ label: 'Sekcja 2 — tytuł', defaultValue: 'Szacunek do każdego zlecenia.', validation: { isRequired: true } }),
        s2Text1: fields.text({ label: 'Sekcja 2 — akapit 1', multiline: true, defaultValue: 'Rozumiemy, że nagrobek jest miejscem pamięci bliskich osób, dlatego do każdego zlecenia podchodzimy z należytą starannością i szacunkiem. W naszej pracy liczy się przede wszystkim precyzja i dbałość o szczegóły.', validation: { isRequired: true } }),
        s2Text2: fields.text({ label: 'Sekcja 2 — akapit 2', multiline: true, defaultValue: 'Stawiamy na uczciwe podejście, terminowość oraz bezpośredni kontakt z klientem na każdym etapie realizacji zlecenia. Naszym celem jest świadczenie usług solidnych, estetycznych i trwałych — tak, aby efekty naszej pracy służyły przez długie lata.' }),
        s3Label: fields.text({ label: 'Sekcja 3 — etykieta', defaultValue: 'Wycena' }),
        s3Title: fields.text({ label: 'Sekcja 3 — tytuł', defaultValue: 'Zdjęcia wystarczą.', validation: { isRequired: true } }),
        s3Text1: fields.text({ label: 'Sekcja 3 — akapit 1', multiline: true, defaultValue: 'W wielu przypadkach do wstępnej wyceny wystarczą zdjęcia przesłane telefonicznie, co pozwala szybko określić zakres prac oraz termin realizacji.', validation: { isRequired: true } }),
        s3Text2: fields.text({ label: 'Sekcja 3 — akapit 2', multiline: true, defaultValue: 'Napisz lub zadzwoń — bezpośredni kontakt na każdym etapie, wycena bezpłatna.' }),
      },
    }),
